...
export default function PostCommentModerationPanel() {
  const actionRef = useRef<ActionType>();
  const [reviewingComment, setReviewingComment] = useState<PostCommentVO | undefined>();
  const [reviewStatus, setReviewStatus] = useState<number>(0);
  const [reviewMessage, setReviewMessage] = useState("");
  const [saving, setSaving] = useState(false);

  const columns: ProColumns<PostCommentVO>[] = [
    {
      title: "回复 ID",
      dataIndex: "id",
      width: 120,
    ...
    <>
      <div className="rounded-[2.2rem] border border-slate-100 bg-slate-50/70 p-5">
        <div className="space-y-2">
          <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-primary">
            ...
            Community Reply Review
          </div>
          <h2 className="text-2xl font-black tracking-tight text-slate-900">社区回复审核</h2>
          <p className="text-sm font-medium leading-6 text-slate-500">
            在社区管理内统一处理自动审核未通过的帖子评论与二级回复，减少后台导航碎片化。
          </p>
        </div>
      </div>

      <div className="rounded-[2.5rem] border border-slate-100 bg-white p-4 shadow-2xl shadow-slate-200/40 sm:p-6">
        <ProTable<PostCommentVO>
          headerTitle={null}
          actionRef={actionRef}
          rowKey="id"
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          setSaving(true);
          try {
            await reviewPostComment({
              id: reviewingComment.id,
              status: reviewStatus,
              reviewMessage,
            });
            message.success("社区回复审核结果已更新");
      ...
      >
        <div className="space-y-6 pt-4">
          <div>
            <div className="text-xs font-black uppercase tracking-[0.2em] text-primary">Review Reply</div>
            <h3 className="mt-2 text-2xl font-black text-slate-900">审核社区回复</h3>
