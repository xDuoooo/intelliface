...
const BankQuestionListControls = ({
  current,
  pageSize,
  total,
  defaultQuery = "",
  ...
  const visibleCountText = useMemo(() => {
    ...
    return `当前显示 ${start}-${end} / ${total} 道题目`;
  ...
  const updateRoute = (query: string, page: number) => {
    ...
    router.push(`${pathname}${queryString ? `?${queryString}` : ""}${hash}`, {
      scroll: false,
    });
  };
  ...
  return (
    <div className="rounded-[2rem] border border-slate-100 bg-white p-4 sm:p-5 shadow-lg shadow-slate-200/30">
      <div className="flex flex-col gap-4">
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 sm:flex-row">
          ...
          <button
            type="submit"
            className="inline-flex h-11 shrink-0 items-center justify-center rounded-2xl bg-primary px-5 text-sm font-bold text-primary-foreground transition-all hover:scale-[1.02] active:scale-95"
          >
            搜索题目
          </button>
        </form>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm font-medium text-slate-500">{visibleCountText}</div>
...
export default BankQuestionListControls;
